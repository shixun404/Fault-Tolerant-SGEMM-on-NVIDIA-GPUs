...
__global__  __launch_bounds__(256) void ft_sgemm_small(int M, int N, int K, float *A, float *B, float *C, float alpha, float beta){
    // ms = ns = ks = 16
    // mw = 8, nw = 16
    // mr = 2, nr = 2
    ...
    *(((float4*)checksum_buffer_B) + tx) = block_level_B_r;
    ...
    checksum += *((float*)(sAB) + offset_A_B + (tx & 15) + 16 * 2);
    ...
    for(k = 0; k < K; k += ks){
        ...
        for(kk = 0; kk < ks; ++kk){
            ...
            res[2] += vec_A[offset_register_kk].y * vec_B[offset_register_kk].x;
