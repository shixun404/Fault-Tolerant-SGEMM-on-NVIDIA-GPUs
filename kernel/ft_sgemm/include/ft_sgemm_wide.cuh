...
__global__  __launch_bounds__(256) void ft_sgemm_wide(int M, int N, int K, float *A, float *B, float *C, float alpha, float beta){
    // ms = 32, ns = 128, ks = 8
    // mw = 16, nw = 64
    // mr = 4,  nr = 8
    ...
    // tile A inner offset.
    // Each thread load (32 * 8) / 128 = 2 floats from A.
    int load_tile_A_num_floats_one_thread = (int)((ms * ks) / blockDim.x);
    // number of threads to load a column of tile A: 32 floats / 2 floats = 16 threads,
    ...
    // tile B inner offset.
    // each thread load (128 * 8) / 128 = 8 floats from B.
    int load_tile_B_num_floats_one_thread = (int)((ns * ks) / blockDim.x);
    // number of threads to load a column of tile B: 128 floats / 8 floats = 16 threads,
    ...
    float2 prefetch_vector_tile_A;
    float4 prefetch_vector_tile_B[2];
    prefetch_vector_tile_A    = *((float2*)A);
    ...
    *(((float2*)buffer_A) + tx) = prefetch_vector_tile_A;
    ...
    *(((float4*)buffer_B) + 2 * tx + 1) = prefetch_vector_tile_B[1];
    ...
    for(k = 0; k < K; k += ks){
        ...
        for(kk = 0; kk < ks; ++kk){
            ...
            res[ 0] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 0].x;
            res[ 1] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 0].y;
            res[ 2] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 0].z;
            res[ 3] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 0].w;

            res[ 4] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 1].x;
            res[ 5] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 1].y;
            res[ 6] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 1].z;
            res[ 7] += vec_A[offset_register_kk].x * vec_B[offset_register_kk * 2 + 1].w;

            res[ 8] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 0].x;
            res[ 9] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 0].y;
            res[10] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 0].z;
            res[11] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 0].w;

            res[12] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 1].x;
            res[13] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 1].y;
            res[14] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 1].z;
            res[15] += vec_A[offset_register_kk].y * vec_B[offset_register_kk * 2 + 1].w;

            res[16] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 0].x;
            res[17] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 0].y;
            res[18] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 0].z;
            res[19] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 0].w;

            res[20] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 1].x;
            res[21] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 1].y;
            res[22] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 1].z;
            res[23] += vec_A[offset_register_kk].z * vec_B[offset_register_kk * 2 + 1].w;

            res[24] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 0].x;
            res[25] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 0].y;
            res[26] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 0].z;
            res[27] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 0].w;

            res[28] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 1].x;
            res[29] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 1].y;
            res[30] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 1].z;
            res[31] += vec_A[offset_register_kk].w * vec_B[offset_register_kk * 2 + 1].w;
        }
    ...
    C_res[ 1] = *((float4 *)(C + 0 + M * 1));
    C_res[ 2] = *((float4 *)(C + 0 + M * 2));
    C_res[ 3] = *((float4 *)(C + 0 + M * 3));
    C_res[ 4] = *((float4 *)(C + 0 + M * 4));
    C_res[ 5] = *((float4 *)(C + 0 + M * 5));
    C_res[ 6] = *((float4 *)(C + 0 + M * 6));
    C_res[ 7] = *((float4 *)(C + 0 + M * 7));
    ...
    *((float4 *)(C + 0 + M * 1)) = C_res[ 1];
    *((float4 *)(C + 0 + M * 2)) = C_res[ 2];
    *((float4 *)(C + 0 + M * 3)) = C_res[ 3];
    *((float4 *)(C + 0 + M * 4)) = C_res[ 4];
    *((float4 *)(C + 0 + M * 5)) = C_res[ 5];
    *((float4 *)(C + 0 + M * 6)) = C_res[ 6];
    *((float4 *)(C + 0 + M * 7)) = C_res[ 7];
}