...
__global__  __launch_bounds__(256) void ft_sgemm_huge(int M, int N, int K, float *A, float *B, float *C, float alpha, float beta){
    // ms = 128, ns = 32, ks = 8
    // mw = 64, nw = 16
    // mr = 8, nr = 4
    ...
    *(((float4*)buffer_B) + 1 * tx + 0) = prefetch_vector_tile_B[0];
    ...
    A_c += __shfl_xor_sync(0xffffffff, A_c, 16, 32);
    ...
    B_r += __shfl_xor_sync(0xffffffff, B_r, 16, 32);
    ...
    *(((float4*)checksum_buffer_B) + tx * 1 + 0) = block_level_B_r[0];
    ...
    for(k = 0; k < K; k += ks){
        ...
        __syncthreads();
        float* s = ((float*)(sAB) + ((idB_warp) * 8) + (idA_warp * 512) + idB_thread + (idA_thread * 128) + 0);
        float* s_ = ((float*)(sAB) + 2048 + ((idA_warp) * 4) + (idB_warp * 1024) + idA_thread + (idB_thread * 128) + 0);
        *(s_ + (0 * 16)) = C_c[0];
        *(s_ + (1 * 16)) = C_c[1];
        *(s_ + (2 * 16)) = C_c[2];
        *(s_ + (3 * 16)) = C_c[3];
        ...
        *(s + (7 * 16)) = C_r[7];
        ...
        if (tx < int(1 * blockDim.x / 2)){
            r_ = *((float4*)((float*)sAB + ((tx& 127) * 1 + 0 ) * 16 + 0));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 127) * 1 + 0 ) * 16 + 4));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 127) * 1 + 0 ) * 16 + 8));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 127) * 1 + 0 ) * 16 + 12));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            ...
            r_ = *((float4*)((float*)sAB + 2048 + ((tx & 127) * 1 + 0) * 16 + 0));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 2048 + ((tx & 127) * 1 + 0) * 16 + 4));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 2048 + ((tx & 127) * 1 + 0) * 16 + 8));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 2048 + ((tx & 127) * 1 + 0) * 16 + 12));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
