...
__global__  __launch_bounds__(128) void ft_sgemm_tall(int M, int N, int K, float *A, float *B, float *C, float alpha, float beta){
    ...
    *(((float2*)buffer_B) + 1 * tx + 0) = prefetch_vector_tile_B[0];
    ...
    B_r += prefetch_vector_tile_B[0].x; B_r += prefetch_vector_tile_B[0].y; 
    ...
    *(((float2*)checksum_buffer_B) + tx * 1 + 0) = block_level_B_r[0];
    ...
    for(k = 0; k < K; k += ks){
        ...
        if(((k+8) %(int(K / 20))) == 0){
            ...
            C_r[4 ] = res[16]; C_r[4 ] += res[17]; C_r[4 ] += res[18]; C_r[4 ] += res[19]; 
            C_r[5 ] = res[20]; C_r[5 ] += res[21]; C_r[5 ] += res[22]; C_r[5 ] += res[23]; 
            C_r[6 ] = res[24]; C_r[6 ] += res[25]; C_r[6 ] += res[26]; C_r[6 ] += res[27]; 
            C_r[7 ] = res[28]; C_r[7 ] += res[29]; C_r[7 ] += res[30]; C_r[7 ] += res[31]; 
            
            C_c[0 ] = res[0 ]; C_c[0 ] += res[4 ]; C_c[0 ] += res[8 ]; C_c[0 ] += res[12]; C_c[0 ] += res[16]; C_c[0 ] += res[20]; C_c[0 ] += res[24]; C_c[0 ] += res[28]; 
            C_c[1 ] = res[1 ]; C_c[1 ] += res[5 ]; C_c[1 ] += res[9 ]; C_c[1 ] += res[13]; C_c[1 ] += res[17]; C_c[1 ] += res[21]; C_c[1 ] += res[25]; C_c[1 ] += res[29]; 
            C_c[2 ] = res[2 ]; C_c[2 ] += res[6 ]; C_c[2 ] += res[10]; C_c[2 ] += res[14]; C_c[2 ] += res[18]; C_c[2 ] += res[22]; C_c[2 ] += res[26]; C_c[2 ] += res[30]; 
            C_c[3 ] = res[3 ]; C_c[3 ] += res[7 ]; C_c[3 ] += res[11]; C_c[3 ] += res[15]; C_c[3 ] += res[19]; C_c[3 ] += res[23]; C_c[3 ] += res[27]; C_c[3 ] += res[31]; 
            
        __syncthreads();
        float* s = ((float*)(sAB) + ((idB_warp) * 4) + (idA_warp * 512) + idB_thread + (idA_thread * 64) + 0);
        float* s_ = ((float*)(sAB) + 1024 + ((idA_warp) * 8) + (idB_warp * 256) + idA_thread + (idB_thread * 64) + 0);
        *(s_ + (0 * 16)) = C_c[0];
        *(s_ + (1 * 16)) = C_c[1];
        *(s_ + (2 * 16)) = C_c[2];
        *(s_ + (3 * 16)) = C_c[3];
        ...
        *(s + (7 * 8)) = C_r[7];
        ...
        if (tx < int(1 * blockDim.x / 2)){
            r_ = *((float4*)((float*)sAB + ((tx& 63) * 2 + 0 ) * 8 + 0));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 63) * 2 + 0 ) * 8 + 4));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 63) * 2 + 1 ) * 8 + 0));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
            r_ = *((float4*)((float*)sAB + ((tx& 63) * 2 + 1 ) * 8 + 4));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
            ...
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 0) * 16 + 0));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 0) * 16 + 4));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 0) * 16 + 8));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 0) * 16 + 12));
            checksum_[0] -= r_.x;
            checksum_[0] -= r_.y;
            checksum_[0] -= r_.z;
            checksum_[0] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 1) * 16 + 0));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 1) * 16 + 4));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 1) * 16 + 8));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
            r_ = *((float4*)((float*)sAB + 1024 + ((tx & 15) * 2 + 1) * 16 + 12));
            checksum_[1] -= r_.x;
            checksum_[1] -= r_.y;
            checksum_[1] -= r_.z;
            checksum_[1] -= r_.w;
        ...
        *((float*)sAB + (1 - int(tx / int(blockDim.x / 2))) * ns + (tx % (int(ws / 2))) * 2 + 1) = checksum_[1];
        __syncthreads();
        tmp_col[0] = (*((float4*)((float*)sAB + (idB_warp * 16 + idB_thread * 4) + 0)));
        tmp_row[0] = (*((float4*)((float*)sAB + ns + (idA_warp * 64 + idA_thread * 8) + 0)));
        tmp_row[1] = (*((float4*)((float*)sAB + ns + (idA_warp * 64 + idA_thread * 8) + 4)));
        ...
        res[16] += int( (fabsf(*((float*)tmp_row + 4)) > err_bound1) && (fabsf(*((float*)tmp_col + 0)) > err_bound1)) * (*((float*)tmp_row + 4));
        res[17] += int( (fabsf(*((float*)tmp_row + 4)) > err_bound1) && (fabsf(*((float*)tmp_col + 1)) > err_bound1)) * (*((float*)tmp_row + 4));
        res[18] += int( (fabsf(*((float*)tmp_row + 4)) > err_bound1) && (fabsf(*((float*)tmp_col + 2)) > err_bound1)) * (*((float*)tmp_row + 4));
        res[19] += int( (fabsf(*((float*)tmp_row + 4)) > err_bound1) && (fabsf(*((float*)tmp_col + 3)) > err_bound1)) * (*((float*)tmp_row + 4));
        res[20] += int( (fabsf(*((float*)tmp_row + 5)) > err_bound1) && (fabsf(*((float*)tmp_col + 0)) > err_bound1)) * (*((float*)tmp_row + 5));
        res[21] += int( (fabsf(*((float*)tmp_row + 5)) > err_bound1) && (fabsf(*((float*)tmp_col + 1)) > err_bound1)) * (*((float*)tmp_row + 5));
        res[22] += int( (fabsf(*((float*)tmp_row + 5)) > err_bound1) && (fabsf(*((float*)tmp_col + 2)) > err_bound1)) * (*((float*)tmp_row + 5));
        res[23] += int( (fabsf(*((float*)tmp_row + 5)) > err_bound1) && (fabsf(*((float*)tmp_col + 3)) > err_bound1)) * (*((float*)tmp_row + 5));
        res[24] += int( (fabsf(*((float*)tmp_row + 6)) > err_bound1) && (fabsf(*((float*)tmp_col + 0)) > err_bound1)) * (*((float*)tmp_row + 6));
        res[25] += int( (fabsf(*((float*)tmp_row + 6)) > err_bound1) && (fabsf(*((float*)tmp_col + 1)) > err_bound1)) * (*((float*)tmp_row + 6));
        res[26] += int( (fabsf(*((float*)tmp_row + 6)) > err_bound1) && (fabsf(*((float*)tmp_col + 2)) > err_bound1)) * (*((float*)tmp_row + 6));
        res[27] += int( (fabsf(*((float*)tmp_row + 6)) > err_bound1) && (fabsf(*((float*)tmp_col + 3)) > err_bound1)) * (*((float*)tmp_row + 6));
        res[28] += int( (fabsf(*((float*)tmp_row + 7)) > err_bound1) && (fabsf(*((float*)tmp_col + 0)) > err_bound1)) * (*((float*)tmp_row + 7));
        res[29] += int( (fabsf(*((float*)tmp_row + 7)) > err_bound1) && (fabsf(*((float*)tmp_col + 1)) > err_bound1)) * (*((float*)tmp_row + 7));
        res[30] += int( (fabsf(*((float*)tmp_row + 7)) > err_bound1) && (fabsf(*((float*)tmp_col + 2)) > err_bound1)) * (*((float*)tmp_row + 7));
        res[31] += int( (fabsf(*((float*)tmp_row + 7)) > err_bound1) && (fabsf(*((float*)tmp_col + 3)) > err_bound1)) * (*((float*)tmp_row + 7));
        __syncthreads();
        }
